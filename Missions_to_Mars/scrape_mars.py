...
def scrape():
    ...
    for image in images:
        ...
        pics_src.append(picture)

    mars_collection["featured_image_url"] = "https://www.jpl.nasa.gov" + pics_src[2] 

    #Mars Facts
    mars_facts_url = "https://space-facts.com/mars/"
    mars_df = pd.read_html(mars_facts_url)[0]
    mars_df.columns = ["Facts","Values"]
    clean_table = mars_df.set_index(["Facts"])
    ...
    soup3 = BeautifulSoup(response_cerberus, 'html.parser')
    cerberus_img = soup3.find_all('div', class_="wide-image-wrapper")
    ...
    schiaparelli_img = soup4.find_all('div', class_="wide-image-wrapper")
    ...
    soup5 = BeautifulSoup(response_syrtis, 'html.parser')
    syrtris_img = soup5.find_all('div', class_="wide-image-wrapper")

    for img in syrtris_img:
        pic_syrtris = img.find('li')
        syrtris_full_img = pic_syrtris.find('a')['href']
    syrtris_title = soup5.find('h2', class_='title').get_text()
    ...
    soup6 = BeautifulSoup(response_valles, 'html.parser')
    valles_img = soup6.find_all('div', class_="wide-image-wrapper")
